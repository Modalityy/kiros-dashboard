    ...
    messages: [
      {
        role: 'system',
        content:
          'You are an expert call evaluator. Determine if the call was successful based on the objectives.\n\nRubric:\n\n{{rubric}}\n\nOnly respond with the evaluation result.',
      },
      {
        role: 'user',
        content:
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            dateTime: {
              type: 'string',
              description: 'Format: YYYY-MM-DDThh:mm:ss+08:00 (Singapore time)',
            },
            bookingType: { type: 'string', enum: ['schedule'] },
            appointmentType: { type: 'string', enum: ['Zoom Meeting'] },
            clientObjective: {
              type: 'string',
              description: "Caller's financial goal or objective.",
            },
          },
        },
      },
